...
ROBERTA_DIR = os.path.join(PHASE3_DIR, "roberta")
ROBERTA_MODEL_DIR = os.path.join(ROBERTA_DIR, "model")
ROBERTA_TOKENIZER_DIR = os.path.join(ROBERTA_DIR, "tokenizer")
ROBERTA_METRICS_DIR = os.path.join(ROBERTA_DIR, "metrics")
ROBERTA_PREDICTIONS_DIR = os.path.join(ROBERTA_DIR, "predictions")
ROBERTA_EMBEDDINGS_DIR = os.path.join(ROBERTA_DIR, "embeddings")
ROBERTA_CONFIG_DIR = os.path.join(ROBERTA_DIR, "config")
...
def setup_phase3_directories() -> None:
    ...
    # roberta/
    create_dir(ROBERTA_MODEL_DIR)
    create_dir(ROBERTA_TOKENIZER_DIR)
    create_dir(ROBERTA_METRICS_DIR)
    create_dir(ROBERTA_PREDICTIONS_DIR)
    create_dir(ROBERTA_EMBEDDINGS_DIR)
    create_dir(ROBERTA_CONFIG_DIR)
...
def load_split(path: str, split_name: str) -> pd.DataFrame:
    ...
    df["needs_sliding_window"] = pd.to_numeric(df["needs_sliding_window"], errors="coerce").fillna(0).astype(int)
    df["too_long_for_single_512_pass"] = pd.to_numeric(df["too_long_for_single_512_pass"], errors="coerce").fillna(0).astype(int)
...
def build_chunk_inputs(
    ...
    use_sliding = (needs_sliding_window_flag == 1) or (token_len > max_length)
    ...
    while start < token_len:
        ...
        full_ids = tokenizer.build_inputs_with_special_tokens(chunk_token_ids)
...
def save_epoch_history(history: List[Dict[str, Any]]) -> None:
    pd.DataFrame(history).to_csv(
        os.path.join(ROBERTA_METRICS_DIR, "training_history.csv"),
        index=False
    )
    save_json(
        os.path.join(ROBERTA_METRICS_DIR, "training_history.json"),
        {"history": history}
    )
...
def save_split_outputs(
    ...
    pred_df.to_csv(
        os.path.join(ROBERTA_PREDICTIONS_DIR, f"{split_name}_predictions.csv"),
        index=False
    )

    np.save(
        os.path.join(ROBERTA_EMBEDDINGS_DIR, f"{split_name}_embeddings.npy"),
        embeddings
    )
    ...
    embedding_meta.to_csv(
        os.path.join(ROBERTA_EMBEDDINGS_DIR, f"{split_name}_embedding_metadata.csv"),
        index=False
    )
...
def save_metrics_file(
    ...
    save_json(
        os.path.join(ROBERTA_METRICS_DIR, f"{split_name}_metrics.json"),
        payload
    )

    pd.DataFrame([metrics]).to_csv(
        os.path.join(ROBERTA_METRICS_DIR, f"{split_name}_metrics.csv"),
        index=False
    )
...
def save_final_artifacts(
    ...
    tokenizer.save_pretrained(ROBERTA_TOKENIZER_DIR)
    ...
    save_json(os.path.join(ROBERTA_CONFIG_DIR, "train_config.json"), config_payload)
    save_json(
        os.path.join(ROBERTA_CONFIG_DIR, "label_map.json"),
        {
...
def main() -> None:
    ...
    for epoch_idx in range(CFG.epochs):
        ...
        if val_metrics["f1"] > best_val_f1:
            ...
            save_json(
                os.path.join(ROBERTA_CONFIG_DIR, "best_checkpoint_info.json"),
                {
    ...
    print("\nReloading best saved RoBERTa model for final train/validation/test inference...")
    ...
    save_json(
        os.path.join(ROBERTA_CONFIG_DIR, "run_summary.json"),
        {
            ...
            "artifacts_saved": {
                ...
                "metrics_dir": ROBERTA_METRICS_DIR,
                "predictions_dir": ROBERTA_PREDICTIONS_DIR,
                "embeddings_dir": ROBERTA_EMBEDDINGS_DIR,
                "config_dir": ROBERTA_CONFIG_DIR,
            },
        }
    )
    ...
    print(ROBERTA_TOKENIZER_DIR)
    print(ROBERTA_METRICS_DIR)
    print(ROBERTA_PREDICTIONS_DIR)
    print(ROBERTA_EMBEDDINGS_DIR)
    print(ROBERTA_CONFIG_DIR)
    print("\nDone.")
